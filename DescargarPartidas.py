...
class MetadataCollector:
        ...
        try:
            ...
            for snap in self.metadata_history:
                ...
                snap['objectives'] = self._build_objectives(
                    frame_ts, team_dragons, team_barons, team_towers,
    ...
    def _detect_teamfight(self, frame, recent_deaths, current_ts) -> dict:
        DISTANCE    = 3000
        TIME_WINDOW = 10000
        ...
        for pid_str, data in pf.items():
            ...
            (team_100 if pid <= 5 else team_200).append({"id": pid - 1, "pos": pos})

        def near_groups(players):
            return [
                [p2["id"] for p2 in players
                 if ((p1["pos"]["x"]-p2["pos"]["x"])**2 + (p1["pos"]["y"]-p2["pos"]["y"])**2)**0.5 <= DISTANCE]
                for p1 in players
                if sum(1 for p2 in players
                       if ((p1["pos"]["x"]-p2["pos"]["x"])**2 + (p1["pos"]["y"]-p2["pos"]["y"])**2)**0.5 <= DISTANCE) >= 4
            ]

        blue_g = near_groups(team_100)
        red_g  = near_groups(team_200)
        recent = [d for d in recent_deaths if current_ts - d["timestamp"] <= TIME_WINDOW]
        in_tf  = set()
        if blue_g and red_g and recent:
            for g in blue_g + red_g:
                in_tf.update(g)
        return {i: i in in_tf for i in range(10)}
...
class SpectatorRecorder:
    ...
    def start_recording(self):
        ...
        log("✅ Modo metadata-only completado")
        return None

    def assemble_rofl(self):
        if not self.chunks:
            log("⚠️ Sin chunks para ensamblar", "WARN")
            return None
        log("\n🔧 Ensamblando .rofl...")
        try:
            meta   = {"gameId": self.game_id, "platformId": self.platform_id,
            ...
            chunk_keys = sorted(self.chunks.keys())
            for i, c in enumerate(chunk_keys):
                nxt = chunk_keys[i+1] if i+1 < len(chunk_keys) else 0
            ...
            off     = 0
            for e in entries:
                payload += struct.pack('<IBIII', e['id'], e['type'], len(e['data']),
                                       e.get('next', 0), off + h_size)
                off += len(e['data'])
            for e in entries:
                payload += e['data']

            enc_b    = self.encryption_key.encode('utf-8')
            p_header = struct.pack('<QIIIIIIH',
                                   self.game_id, self.game_length_ms,
                                   len(self.keyframes), len(self.chunks),
                                   self.end_startup_chunk_id, self.start_game_chunk_id,
                                   60000, len(enc_b)) + enc_b

            h_len  = 288
            m_off  = h_len;          m_len  = len(meta_b)
            ph_off = m_off + m_len;  ph_len = len(p_header)
            p_off  = ph_off + ph_len
            f_len  = p_off + len(payload)

            f_header = (b'RIOT\x00\x00' + b'\x00' * 256 +
                        struct.pack('<HIIIIIII',
                                    h_len, f_len, m_off, m_len,
                                    ph_off, ph_len, p_off, 0))

            ts   = datetime.now().strftime("%Y%m%d_%H%M%S")
            path = os.path.join(SAVE_PATH, f"KR_{self.game_id}_{ts}.rofl")
            with open(path, 'wb') as f:
                f.write(f_header); f.write(meta_b)
                f.write(p_header); f.write(payload)
